...
function findPromotorByLoja(rede, loja) {
    if (!APP_DATA[rede]) return null;
    for (const prom in APP_DATA[rede]) {
        if (APP_DATA[rede][prom].includes(loja)) {
            return prom;
        }
    }
    return null;
...
function loadAndPopulateDropdowns() {
    ...
    selectRede.innerHTML = '<option value="" disabled selected>Selecione a sua Rede</option>';
    Object.keys(APP_DATA).forEach(rede => {
...
function populateLoja(rede) {
    ...
    if (rede && APP_DATA[rede]) {
        const allLojas = [];
        for (const prom in APP_DATA[rede]) {
            APP_DATA[rede][prom].forEach(loja => {
                if (!allLojas.includes(loja)) {
                    allLojas.push(loja);
                }
            });
        }
